...
class PatternMatchingTransactionalPersistedEventHandlerTest {
    @Test
    void test_eventhandler_method_matching() throws JsonProcessingException {
        var eventHandler = new TestPatternMatchingTransactionalPersistedEventHandler();
        var orderId      = OrderId.random();
        var unitOfWork   = Mockito.mock(UnitOfWork.class);
        ...
        // Private method WITH SubscriptionEventHandler annotation but missing the UnitOfWork 2nd argument,
        // hence shouldn't match
        var productId       = ProductId.random();
        var productAdded = new ProductEvent.ProductAdded(productId);
        assertThatThrownBy(() -> eventHandler.handle(typedEvent(productId,
                                                                productAdded,
                                                                0),
                                                     unitOfWork))
                .isExactlyInstanceOf(IllegalArgumentException.class);
    ...
    private static class TestPatternMatchingTransactionalPersistedEventHandler extends PatternMatchingTransactionalPersistedEventHandler {
        ...
        @SubscriptionEventHandler
        private void handle(OrderEvent.ProductOrderQuantityAdjusted productOrderQuantityAdjusted, UnitOfWork unitOfWork, Object unused) {
            requireNonNull(unitOfWork, "No unitOfWork provided");
        ...
        private void handle(OrderEvent.OrderAccepted orderAccepted, UnitOfWork unitOfWork) {
            requireNonNull(unitOfWork, "No unitOfWork provided");
        ...
        @SubscriptionEventHandler
        private void handle(ProductEvent.ProductAdded productAdded) {
