...
import dk.trustworks.essentials.shared.measurement.LogThresholds;
...
@Testcontainers
class EventStoreSubscriptionManager_exclusivelySubscribeToAggregateEventsAsynchronously_IT {
    ...
    void subscribe() {
        ...
        var productsSubscription = eventStoreSubscriptionManagerNode1.exclusivelySubscribeToAggregateEventsAsynchronously(
        ...
        var ordersSubscription = eventStoreSubscriptionManagerNode1.exclusivelySubscribeToAggregateEventsAsynchronously(
        ...
        var totalNumberOfProductEvents = testEvents.get(PRODUCTS)
                                                   ...
                                                   .reduce(Integer::sum)
                                                   .get();
        System.out.println("subscribe - Total number of Product Events: " + totalNumberOfProductEvents);
        ...
        var totalNumberOfOrderEvents = testEvents.get(ORDERS)
                                                 ...
                                                 .reduce(Integer::sum)
                                                 .get();
        System.out.println("subscribe - Total number of Order Events: " + totalNumberOfOrderEvents);
        Awaitility.waitAtMost(Duration.ofSeconds(10))
        ...
        testEvents.forEach((aggregateType, aggregatesAndEvents) -> {
            aggregatesAndEvents.forEach((aggregateId, events) -> {
                ...
                if (count == 80) {
                    ...
                    Awaitility.waitAtMost(Duration.ofSeconds(5))
                              .untilAsserted(() -> assertThat(ordersSubscription.isActive()).isTrue());
    ...
    void test_with_resubscription() {
        ...
        assertThat(ordersSubscription.get().currentResumePoint().get().getResumeFromAndIncluding()).isEqualTo(lastEventOrder.globalEventOrder().increment()); // When the subscriber is stopped we store the next global event order
    ...
    private EventStoreSubscription createOrderSubscription(ConcurrentLinkedDeque<PersistedEvent> orderEventsReceived) {
        return eventStoreSubscriptionManagerNode1.exclusivelySubscribeToAggregateEventsAsynchronously(
                SubscriberId.of("OrdersSub1"),
                ORDERS,
                GlobalEventOrder.FIRST_GLOBAL_EVENT_ORDER,
                Optional.empty(),
