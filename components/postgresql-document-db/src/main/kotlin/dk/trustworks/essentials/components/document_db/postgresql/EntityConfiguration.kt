...
import dk.trustworks.essentials.shared.reflection.BoxedTypes
...
/**
 * [VersionedEntity] Configuration class used by [dk.trustworks.essentials.components.document_db.postgresql.PostgresqlDocumentDbRepository]
 *
...
class EntityConfiguration<ID, T : VersionedEntity<ID, T>>(private val entityClass: KClass<T>) {
    ...
    fun versionProperty(): KMutableProperty1<T, dk.trustworks.essentials.components.document_db.Version> {
    ...
    companion object {
         ...
         * e.g. by [dk.trustworks.essentials.components.document_db.DocumentDbRepositoryFactory.create], will call the [dk.trustworks.essentials.components.foundation.postgresql.PostgresqlUtil.checkIsValidTableOrColumnName]
         * to check table name (see [dk.trustworks.essentials.components.document_db.annotations.DocumentEntity]) and JSON property names, that are resulting from the JSON serialization of the concrete [VersionedEntity] being persisted,
         ...
         *
         * This method attempts to check all property name against [dk.trustworks.essentials.components.foundation.postgresql.PostgresqlUtil.checkIsValidTableOrColumnName]
         *
         ...
         *
         * Check is it's a [Number], [CharSequence], [dk.trustworks.essentials.shared.reflection.BoxedTypes.isBoxedType],
         * [dk.trustworks.essentials.shared.reflection.BoxedTypes.isPrimitiveType] or if the type belongs to these package names:
