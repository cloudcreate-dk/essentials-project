...
import dk.trustworks.essentials.kotlin.types.BigIntegerValueType
...
/**
 * Generic [ColumnMapper] for [BigIntegerValueType]'s
 *
...
abstract class BigIntegerValueTypeColumnMapper<T : BigIntegerValueType<T>> : ColumnMapper<T?> {
    ...
    override fun map(r: ResultSet, columnNumber: Int, ctx: StatementContext): T? {
        val value = r.getBytes(columnNumber)
        return if (value == null) null else concreteType.primaryConstructor!!.call(BigInteger(value))
    }
}
