...
/**
 * Generic [ColumnMapper] for [LocalTimeValueType]'s
 *
...
abstract class LocalTimeValueTypeColumnMapper<T : LocalTimeValueType<T>> : ColumnMapper<T?> {
    ...
    override fun map(r: ResultSet, columnNumber: Int, ctx: StatementContext): T? {
        val value = r.getTime(columnNumber)
        return if (value == null) null else concreteType.primaryConstructor!!.call(value.toLocalTime())
    }
}
