...
/**
 * Generic {@link ColumnMapper} for {@link IntegerType}'s<br>
 * <br>
 ...
 */
public abstract class IntegerTypeColumnMapper<T extends IntegerType<T>> implements ColumnMapper<T> {
    private final Class<T> concreteType;

    @SuppressWarnings("unchecked")
    public IntegerTypeColumnMapper() {
        concreteType = (Class<T>) GenericType.resolveGenericTypeOnSuperClass(this.getClass(),
                                                                             0);
    }

    public IntegerTypeColumnMapper(Class<T> concreteType) {
    ...
    public T map(ResultSet r, int columnNumber, StatementContext ctx) throws SQLException {
        return SingleValueType.from(r.getInt(columnNumber),
                                    concreteType);
    }
}
