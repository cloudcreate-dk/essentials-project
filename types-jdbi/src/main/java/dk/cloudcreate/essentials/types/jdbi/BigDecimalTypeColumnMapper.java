...
/**
 * Generic {@link ColumnMapper} for {@link BigDecimalType}'s<br>
 * <br>
 ...
 * }}</pre>
 *
 * @param <T> the concrete {@link BigDecimalType} this instance is mapping
 */
public abstract class BigDecimalTypeColumnMapper<T extends BigDecimalType<T>> implements ColumnMapper<T> {
    private final Class<T> concreteType;

    @SuppressWarnings("unchecked")
    public BigDecimalTypeColumnMapper() {
        concreteType = (Class<T>) GenericType.resolveGenericTypeOnSuperClass(this.getClass(),
                                                                             0);
    }

    public BigDecimalTypeColumnMapper(Class<T> concreteType) {
