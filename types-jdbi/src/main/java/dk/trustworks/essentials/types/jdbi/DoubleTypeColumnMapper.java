...
/**
 * Generic {@link ColumnMapper} for {@link DoubleType}'s<br>
 * <br>
 * Example of a concrete mapper:
 * <pre>{@code
 * public final class SomeTypeArgumentFactory extends DoubleTypeColumnMapper<SomeType> {
 * }}</pre>
 * @param <T> the concrete {@link DoubleType} this instance is mapping
 */
public abstract class DoubleTypeColumnMapper<T extends DoubleType<T>> implements ColumnMapper<T> {
    private final Class<T> concreteType;

    @SuppressWarnings("unchecked")
    public DoubleTypeColumnMapper() {
        concreteType = (Class<T>) GenericType.resolveGenericTypeOnSuperClass(this.getClass(),
                                                                             0);
    }

    public DoubleTypeColumnMapper(Class<T> concreteType) {
    ...
    public T map(ResultSet r, int columnNumber, StatementContext ctx) throws SQLException {
        return SingleValueType.from(r.getDouble(columnNumber),
                                    concreteType);
    }
}
