...
/**
 * Generic {@link ColumnMapper} for {@link FloatType}'s<br>
 * <br>
 * Example of a concrete mapper:
 * <pre>{@code
 * public final class SomeTypeArgumentFactory extends FloatTypeColumnMapper<SomeType> {
 * }}</pre>
 *
 * @param <T> the concrete {@link DoubleType} this instance is mapping
 */
public abstract class FloatTypeColumnMapper<T extends FloatType<T>> implements ColumnMapper<T> {
    private final Class<T> concreteType;

    @SuppressWarnings("unchecked")
    public FloatTypeColumnMapper() {
        concreteType = (Class<T>) GenericType.resolveGenericTypeOnSuperClass(this.getClass(),
                                                                             0);
    }

    public FloatTypeColumnMapper(Class<T> concreteType) {
    ...
    public T map(ResultSet r, int columnNumber, StatementContext ctx) throws SQLException {
        return SingleValueType.from(r.getFloat(columnNumber),
                                    concreteType);
    }
}
