...
public class TransferTime extends OffsetDateTimeType<TransferTime> {
    ...
    public static TransferTime now() {
        // Using withNano(0) since the OffsetDateTimeType logical-type only has milliseconds resolution
        return new TransferTime(OffsetDateTime.now(Clock.systemUTC()).withNano(0));
    }
}
